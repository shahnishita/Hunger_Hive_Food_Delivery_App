...
// Hardcoded restaurant location (I-Space IT Park, Bavdhan, Pune)
...
const LiveOrderTracking = () => {
  ...
  const calculateDelivery = (sourceLoc, destLoc) => {
    ...
    const charges = Math.ceil(dist * 1);
    ...
    console.log("⏱️ Estimated Time:", timeMinutes.toFixed(0), "minutes");
    console.log("💰 Extra Charges: ₹", charges);
  };
  ...
  useEffect(() => {
    const fetchDeliveryBoys = async () => {
      try {
        ...
        console.log("📦 Fetched Delivery Boys from API:", res.data);

        // Check if res.data is array or has data array
        if (Array.isArray(res.data)) {
          setDeliveryBoys(res.data);
        } else if (res.data && Array.isArray(res.data.data)) {
          setDeliveryBoys(res.data.data);
        } else {
          console.warn("⚠️ Unexpected delivery boys response format", res.data);
          setDeliveryBoys([]); // fallback empty array
        }
      } catch (err) {
        console.error("❌ Failed to fetch delivery boys:", err.message);
        setDeliveryBoys([]); // fallback empty array on error
      }
    };
  ...
  // Calculate delivery details and assign boy
  useEffect(() => {
    (async () => {
      try {
        ...
        if (deliveryBoys.length > 0) {
          ...
          const chosenBoy = deliveryBoys[randomIndex];
          console.log("✅ Assigned Delivery Boy:", chosenBoy);
          setAssignedBoy(chosenBoy);
        } else {
          console.warn("⚠️ No delivery boys available.");
          setAssignedBoy(null);
        }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Show delivery boys with dummy coordinates */}
        {deliveryBoys.map((boy, idx) => {
          // Assign dummy lat/lon offset near Pune to avoid overlapping markers
          const lat = 18.5205 + 0.02 + idx * 0.01; // a bit north of restaurant
      ...
      {assignedBoy ? (
        ...
        >
          <p>
            <strong>Name:</strong> {assignedBoy.name}
          </p>
          <p>
            <strong>Phone:</strong> {assignedBoy.phone}
          </p>
          <p>
            <strong>From:</strong> {assignedBoy.address || "N/A"}
          </p>
          <p>
            <strong>Instructions:</strong> {assignedBoy.instructions || "None"}
          </p>
        </div>
      ) : (
      ...
      {distance && estimatedTime ? (
        ...
        >
          ...
          <p>
            <strong>Extra Charges:</strong> ₹{extraCharges}
          </p>
        </div>
      ) : (
